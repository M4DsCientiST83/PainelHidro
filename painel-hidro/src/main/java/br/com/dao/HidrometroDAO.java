...
public class HidrometroDAO {
    public void salvar(Hidrometro hidro) {
        salvar(hidro, 'A'); // Tipo padrão A
    }

    public void salvar(Hidrometro hidro, char tipo) {
        String sql = """
                    INSERT INTO hidrometro (id, tipo)
                    VALUES (?, ?)
                """;
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Erro ao salvar hidrômetro", e);
        }
    }
    ...
    public char buscarTipo(int idHidrometro) {
        String sql = "SELECT tipo FROM hidrometro WHERE id = ?";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                return (tipo != null && !tipo.isEmpty()) ? tipo.charAt(0) : 'A';
            } else {
                throw new RuntimeException("Hidrômetro não encontrado");
            }
        } catch (SQLException e) {
